...
- `--model`: Đường dẫn đến model (bắt buộc)
- `--input-dir`: Thư mục chứa file JSON prompts (bắt buộc)
- `--output-dir`: Thư mục lưu kết quả (bắt buộc)
- `--batch-size`: Batch size (mặc định: 1, tăng nếu có nhiều VRAM)
...
- `--no-4bit`: Tắt 4-bit quantization (không khuyến nghị, dùng nhiều VRAM)
- `--quant-type`: Loại quantization: nf4 (mặc định) hoặc fp4
